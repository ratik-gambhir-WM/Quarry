...
use crate::{
    adapters::office::converter::OfficeConverter,
    app::config::DataRoomConfig,
    domains::{
        data_rooms::local_source::{
            build_directory_node, read_pdf, resolve_relative_file, validate_pdf_bytes,
            DataRoomTreeNode,
        },
        deals::DataRoomSourceReader,
    },
...
impl DataRoomService {
    ...
    async fn deal_data_room_root(&self, deal_id: &str) -> ServiceResult<Option<PathBuf>> {
        if let Some(local_root) = self.deals.local_root(deal_id.to_string()).await? {
            return Ok(Some(PathBuf::from(local_root)));
        }
