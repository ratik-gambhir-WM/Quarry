...
use crate::{
    ...
    domains::documents::ingestion::{
        job_service::DocumentJobService,
        service::{DocumentIngestionService, ProcessDocumentsResponse, UploadedDocument},
    },
    shared::{
        file_policy::{MAX_FILE_BYTES, MAX_TOTAL_REQUEST_FILE_BYTES},
...
#[derive(Clone)]
pub(super) struct DocumentIngestionHttpState {
...
pub(super) async fn process_documents_handler(
    State(state): State<DocumentIngestionHttpState>,
...
pub(super) async fn start_process_file_handler(
    State(state): State<DocumentIngestionHttpState>,
    Path(deal_id): Path<String>,
    multipart: Multipart,
...
pub(super) async fn process_document_job_events_handler(
    State(state): State<DocumentIngestionHttpState>,
