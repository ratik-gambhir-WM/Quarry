...
export function SidebarFrame({
  ...
  const subtitle = profileDeal
    ? profileDeal.room.name
    : profileSubtitle ?? "Focus: Project Alpha";

  return (
    <aside
      className={`hidden h-full shrink-0 overflow-hidden border-r border-outline-variant/70 bg-[#f7f7f7] text-sidebar-text backdrop-blur-md transition-[width] duration-300 ease-out lg:flex [font-family:var(--font-sidebar)] [html[data-theme=dark]_&]:bg-[#141414] ${collapsed ? "w-20" : "w-72"}`}
    >
      <div className="flex h-full min-h-0 w-full flex-col">
        <div
          className={
            alignedHeader
              ? `flex h-16 shrink-0 items-center justify-between gap-1 border-b border-outline-variant/70 ${collapsed ? "px-2" : "px-4"}`
        ...
        >
          ...
          >
            <WestMonroeMark
              className={collapsed ? "h-[1.4rem] w-[1.4rem]" : "h-8 w-8"}
              framed
            />
            {collapsed ? null : (
              <h1 className="text-[13px] font-semibold leading-5 text-sidebar-active">Quarry</h1>
            )}
          </NavLink>
        ...
        <div className={`flex min-h-0 flex-1 flex-col ${collapsed ? "p-2" : "p-4"}`}>
          <div
            className={`min-h-0 flex-1 overflow-y-auto pb-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${alignedHeader ? "pt-2" : "pt-6"} ${
          ...
          <div className="shrink-0 border-t border-outline-variant/70 pt-2">
            <div className="relative">
              {!collapsed && profileMenuOpen ? (
                <ProfilePreferences email={email} navigationState={navigationState} />
              ) : null}
              <button
                ...
                type="button"
              >
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-secondary-fixed-dim text-white">
                  <span className="text-[12px] font-semibold">{teamLabel.slice(0, 1)}</span>
                </div>
                {collapsed ? null : (
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-[11px] font-normal leading-4 text-sidebar-muted">
                      {subtitle}
                    </p>
                  </div>
                )}
