...
This package is Quarry's standalone Node/Express Diligence Studio service. Its preview renderer is
the sibling React application in `../frontend`. It keeps its PowerPoint
...
Use `npm start` for the non-watching server, `npm run typecheck` for TypeScript validation, and
`npm test` for the server and CLI suites. Run every command from this directory; the package has
its own manifest, lockfile, and installed dependency tree.
...
shown. The renderer blocks requests to origins other than the configured frontend origin. Run the
frontend and API together during local imports; production must expose the internal preview route
at `QUARRY_TEMPLATE_PREVIEW_RENDER_URL`.

## Provenance

Quarry copied the tracked `server/` tree from Diligence Studio `main` at commit
`9aea6fd6e75ddc3ddb01e05b4b326d2dcc7248cd`. This directory is now Quarry-owned and has no runtime
or build dependency on the former repository.
