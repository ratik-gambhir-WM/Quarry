...
export function TransactionTypePicker({ error = "", onChange, value }: TransactionTypePickerProps) {
  ...
  return (
    <ModalField
      ...
      labelId="add-transaction-type-label"
    >
      <div ref={pickerRef}>
        <button
          aria-expanded={open}
          ...
          className={`flex w-full items-center justify-between rounded-2xl border border-outline-variant bg-surface-container-lowest px-4 py-3 text-left text-[14px] outline-none transition focus:border-primary-container focus:ring-4 focus:ring-primary-fixed/40 ${
